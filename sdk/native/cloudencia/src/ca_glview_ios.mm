...
-(void)setPAR:(int)numerator denominator:(int)denominator {
    @synchronized(self) {
        if (!_parNumerator || _parDenominator) {
            NSLog(@"Invalid PAR:%d/%d", numerator, denominator);
            assert(0);
        }
...
- (void)updateSizes {
    @synchronized(self) {
        ...
        rcSrc = CorrectAspectRatio(rcSrc, { _parNumerator, _parDenominator });
        RECT rcViewport = LetterBoxRect(rcSrc, { 0, 0, _textureWidth, _textureHeight });
        
        _viewportW = Width(rcViewport) ;
        _viewportH = Height(rcViewport);
        _viewportX = rcViewport.left;
        _viewportY = rcViewport.top;
    }
}
...
- (void)render:(CADisplayLink*)displayLink {
    if ((!_animating && !_stopping) || [UIApplication sharedApplication].applicationState != UIApplicationStateActive) {
        return;
    }
    @synchronized(self) {
