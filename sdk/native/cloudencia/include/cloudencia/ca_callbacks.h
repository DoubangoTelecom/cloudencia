...
public:
	...
	virtual bool onStateChanged(enum CANetState_e newState, std::string description = "")const = 0;
	virtual bool onPassthroughData(const void* pcDataPtr, size_t nDataSize)const = 0;

private:
	static bool raiseStateChanged(const CAObjWrapper<CACallbackNet* >&oThis, enum CANetState_e newState, std::string description = "");
